...
def post_analysis(config_name: str, type: str = "mixed"):
    """
    ...
    Args:
        ...
        type (str, optional): The type of the internal coordinate to plot. Defaults to "mixed". Can be "angle" or "bond".
    ...
    a, bins, _ = axs[0].hist(y_true, bins=150, alpha=0.4, label="True", color="xkcd:blue", edgecolor="black", weights=y_true_weights)
    b, _, _ = axs[0].hist(y_pred, bins=bins, alpha=0.4, label="Predicted", color="xkcd:green", edgecolor="black", weights=y_pred_weights)
    ...
    plt.savefig(os.path.join("post_analysis", f"all_histograms_{config_name}.pdf"))


post_analysis("prod_bond", "bond")
post_analysis("prod_angle", "angle")
post_analysis("prod")