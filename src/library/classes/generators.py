...
def get_output_scale_factor(ic_index: int):
    """
    ...
    BOND_SCALE = 1 / 10
    ANGLE_SCALE = 1 / (2 * np.pi)
    DIHEDRAL_SCALE = 1 / (2 * np.pi)

    if ic_type == "bond":
        return OUTPUT_SCALE * BOND_SCALE
    elif ic_type == "angle":
        return OUTPUT_SCALE * ANGLE_SCALE
    elif ic_type == "dihedral":
        return OUTPUT_SCALE * DIHEDRAL_SCALE
    else:
...
class FICDataGenerator(BaseDataGenerator):
    ...
    def __gen_item__(self, idx):
        ...
        for i in range(self.batch_size):
            ...
            scaled_output_ic_value = output_ic_value * get_output_scale_factor(self.ic_index)
