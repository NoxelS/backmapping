...
AT_BOUNDING_BOX_RELATION_PATH = os.path.join(config(Keys.DATA_PATH), "box_sizes_at.csv")  # This is generated by the generate_molecule_data_fast.py script
NEIGHBOURHOOD_PATH = os.path.join(config(Keys.DATA_PATH), "neighborhoods.csv")  # This is generated by the generate_molecule_data_fast.py script
...
ABSOLUTE_POSITION_SCALE_X = 200.0              # This factor is used to scale down bead positions
ABSOLUTE_POSITION_SCALE_Y = 30.0               # This factor is used to scale down atom positions
...
def add_absolute_positions(atoms, output_matrix, batch_index, box, target_atom=None, preset_position_origin=None, position_scale=ABSOLUTE_POSITION_SCALE_X):
    """
...
class AbsolutePositionsNeigbourhoodGenerator(BackmappingBaseGenerator):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            augmentation,
            data_usage,
            cache_name="APN_" + ("validation" if validation_mode else "training") + "_" + atom_name + "_" + str(batch_size) + "_cache"
        )
        ...
        if not is_output_matrix_healthy(Y) or not is_output_matrix_healthy(X):
            if not is_output_matrix_healthy(Y):
                ...
                for i in range(Y.shape[0]):
                    if not is_output_matrix_healthy(Y[i:i+1, :, :, :]):
