from memdof import parse_PTB, calc_IDOF, parse_topology, ExtendedTopologyInfo
...
def generate_extended_topology() -> ExtendedTopologyInfo:
    ...
    topology_info = parse_topology(PATH_TO_TOPOLOGY)

    # Parse the PDB file to extract the structure
    structure, pbc = parse_PTB(PATH_TO_PDB)

    # Calculate the internal degrees of freedom
    extended_topology_info: ExtendedTopologyInfo = calc_IDOF(structure, pbc, topology_info, quiet=False, create_plots=True, plots_path=TOPOLOGY_PLOT_PATH, create_csv=True, csv_path=CSV_PATH)

    # Save the extended topology information
    with open(f"{config(Keys.DATA_PATH)}/training/DOPC/extended_topology.pkl", 'wb') as fp:
