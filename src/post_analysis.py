...
"""
plot_loss_atom_name(predictions, "loss").savefig(
    gen_path("loss", "loss_atom_name.png"), **savefig_kwargs
)
plot_loss_atom_name(predictions, "mae").savefig(
    gen_path("loss", "mae_atom_name.png"), **savefig_kwargs
)
...
"""
plot_cluster_hist(2).savefig(
    gen_path("training", "training_loss.png"), **savefig_kwargs
)
...
plot_cluster_hist(5).savefig(
    gen_path("training", "training_val_acc.png"), **savefig_kwargs
)
plot_cluster_hist(6).savefig(
    gen_path("training", "training_val_loss.png"), **savefig_kwargs
)
plot_cluster_hist(7).savefig(
    gen_path("training", "training_val_mae.png"), **savefig_kwargs
)
...
"""
[
    plot_bond_length_distribution(analysis_data, bond).savefig(
        gen_path("bonds", "lengths", f"bond_length_{bond[0]}_{bond[1]}.png"),
        **savefig_kwargs,
    )
...
"""
plot_total_bond_length_distribution(analysis_data).savefig(
    gen_path("bonds", "total_bond_length.png"), **savefig_kwargs
)
...
"""
plot_total_bond_length_distribution(
    analysis_data, skip_atoms=["C12", "C13", "C14"]
).savefig(
    gen_path("bonds", "total_bond_length_ignoring_bad_models.png"), **savefig_kwargs
)
...
"""
[
    plot_molecule(analysis_data, i).savefig(
        gen_path(f"molecules", f"mol_{i}.png"), **savefig_kwargs
    )
    for i in range(25)
]
...
"""
[
    plot_bond_angle_distribution(analysis_data, bond1, bond2).savefig(
        gen_path(
            f"bonds",
            "angles",
            f"bond_angle_{bond1[0]}_{bond1[1]}_{bond2[0]}_{bond2[1]}.png",
        ),
        **savefig_kwargs,
    )
    for bond1, bond2 in DOPC_AT_BAB
]
...
"""
plot_total_angle_distribution(analysis_data, DOPC_AT_BAB).savefig(
    gen_path(f"bonds", f"total_bond_angles.png"), **savefig_kwargs
)
...
"""
[
    plot_coordinates_distribution(analysis_data, atom, "x").savefig(
        gen_path("positions", "coordinates", f"coordinates_{atom}_x.png"),
        **savefig_kwargs,
    )
    for atom in ATOM_NAMES_TO_FIT
]
[
    plot_coordinates_distribution(analysis_data, atom, "y").savefig(
        gen_path("positions", "coordinates", f"coordinates_{atom}_y.png"),
        **savefig_kwargs,
    )
    for atom in ATOM_NAMES_TO_FIT
]
[
    plot_coordinates_distribution(analysis_data, atom, "z").savefig(
        gen_path("positions", "coordinates", f"coordinates_{atom}_z.png"),
        **savefig_kwargs,
    )
    for atom in ATOM_NAMES_TO_FIT
]
...
[
    molecule_to_ptb(
        analysis_data,
        0,
        gen_path("PDB", f"mol_{i}_true.pdb"),
        gen_path("PDB", f"mol_{i}_pred.pdb"),
        gen_path("PDB", f"mol_{i}_CG.pdb"),
    )
    for i in range(25)
]
...
"""
[
    plot_radial_distribution_function(analysis_data, atom_name).savefig(
        gen_path("positions", "radial_distribution", f"g_{atom_name}.png"),
        **savefig_kwargs,
    )
    for atom_name in ATOM_NAMES_TO_FIT
]
