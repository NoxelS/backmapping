...
for (( i=0; i<$blength; i+=4 )); do
    ...
    sbatch --exclude=fang1,fang51,fang52,fang53,fang54 --gres=gpu:1 --job-name=PB$i --exclusive=user --mem=0 --output=./jobs/logs/$SLURM_JOBID/bonds/$i.log --error=./jobs/logs/$SLURM_JOBID/bonds/$i.err --wrap="jobs/train_single_no_purge.sh smaug_bond $ic1 $ic2 $ic3 $ic4"
done
...
for (( i=0; i<$alength; i+=4 )); do
    ...
    sbatch --exclude=fang1,fang51,fang52,fang53,fang54 --gres=gpu:1 --job-name=PA$i --exclusive=user --mem=0 --output=./jobs/logs/$SLURM_JOBID/angles/$i.log --error=./jobs/logs/$SLURM_JOBID/angles/$i.err --wrap="jobs/train_single_no_purge.sh smaug_angle $ic1 $ic2 $ic3 $ic4"
done
...
for (( i=0; i<$dlength; i+=4 )); do
    ...
    sbatch --exclude=fang1,fang51,fang52,fang53,fang54 --gres=gpu:1 --job-name=PD$i --exclusive=user --mem=0 --output=./jobs/logs/$SLURM_JOBID/dihedrals/$i.log --error=./jobs/logs/$SLURM_JOBID/dihedrals/$i.err --wrap="jobs/train_single_no_purge.sh smaug_angle $ic1 $ic2 $ic3 $ic4"
done