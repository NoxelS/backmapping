...
srun python -u src/train.py -v --config $1 $2 &

if [ "$3" != "" ]; then
    srun python -u src/train.py -v --config $1 $3 &
fi
if [ "$4" != "" ]; then
    srun python -u src/train.py -v --config $1 $4 &
fi
if [ "$5" != "" ]; then
    srun python -u src/train.py -v --config $1 $5 &
fi
